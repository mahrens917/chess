...
def runSanAndPgnTests : IO Unit := do
  ...
  let tryMoveAfterResult := Parsing.applySAN finalState "Kh7"
  match tryMoveAfterResult with
...
unsafe def runDiscoveredCheckTests : IO Unit := do
  ...
  expectBool "rook gives check on e2" (inCheck state.board Color.Black) true
  match Parsing.applySAN state "Re4" with
  | .ok afterMove =>
      expectBool "rook gives check on e4" (inCheck afterMove.board Color.Black) true
...
unsafe def runPinnedPieceTests : IO Unit := do
  let pinnedFen := "4k3/8/8/3r4/8/3B4/8/3K4 w - - 0 1"
  let state ←
  ...
  let absolutePinFen := "4k3/8/8/3r4/8/3N4/8/3K4 w - - 0 1"
  let absPinState ←
